...
class JustWaypointsMenuDelegate extends WatchUi.Menu2InputDelegate {
    ...
    function onSelect(item) {
        ...
        switch (item.getId()) {
            case :menu_add: {
                System.println("Add");
                WatchUi.pushView(_addNewWaypointMenu, new AddMenuDelegate(), WatchUi.SLIDE_IMMEDIATE);
                break;
            }
            ...
            case :menu_clean_storage: {
                ...
                WatchUi.pushView(new WatchUi.Confirmation(_clearStorageConfirmText), new ClearStorageConfirmationDelegate(), WatchUi.SLIDE_IMMEDIATE);
                break;
            }
            case :menu_exit_app: {
